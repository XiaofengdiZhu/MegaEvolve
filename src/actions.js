import { global, save, webWorker, keyMultiplier, keyMap, srSpeak, sizeApproximation, p_on, support_on, gal_on, quantum_level, tmp_vars, setupStats, virtualElement, virtualTree } from './vars.js';
        ...
        Object.keys(costs).forEach(function (res){
            if (res === 'Custom'){
                ...
                if (res_cost > 0){
                    ...
                    empty = false;
                    let display_cost = sizeApproximation(res_cost,1);
                    cost.append($(`<div class="${color} res-${res}" data-${res}="${res_cost}">${label}: ${display_cost}${aria}</div>`));
                }
            }
            else if (res !== 'Morale' && res !== 'Army' && res !== 'Bool'){
                let res_cost = costs[res]();
